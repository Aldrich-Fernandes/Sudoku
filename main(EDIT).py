...
def LoadPuzzleFile(PuzzleName, Puzzle):
	try:
		Line = 0
		FileIn = open(f"{PuzzleName}.txt", 'r',    encoding="utf-8-sig")
...
def LoadSolution(PuzzleName, Solution):
	...
	OK = True
	try:
		FileIn = open(f"{PuzzleName}S.txt", 'r', encoding="utf-8-sig")
...
def MirrorImage(Puzzle, Solution): #flips vertically (i.e. column 9 becomes column 1)
	try:
		for x, line in enumerate(Puzzle):
			CellInfo = list(line)
			CellInfo[1] = str(10 - int(CellInfo[1]))
			Puzzle[x] = "".join(CellInfo)
	except: None
	
	try:
		for x, line in enumerate(Solution):
			Solution[x] = line[::-1]
	except: None
...
def LoadPuzzle(PuzzleGrid, Puzzle, Answer, Solution):
	...
	if OK:
		Puzzle, Solution = MirrorImage(Puzzle, Solution) # Task 1
...
def IsProtectedCell(Puzzle, CellInfo): #task 3
	try:
		for line in Puzzle:
			if line[:-1] == CellInfo[:-1]:
				return True
	except: return False
	return False
...
def SolvePuzzle(PuzzleGrid, Puzzle, Answer):
	...
	if PuzzleGrid[0][0] != 'X':
		...
		while CellInfo != EMPTY_STRING:
			InputError = False
			if len(CellInfo) != 3 or IsProtectedCell(Puzzle, CellInfo):
				InputError = True
			else:
				Digit = CellInfo[2]
				try:
